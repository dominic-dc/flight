								...
								<td>
									<div class="row">
										...
										<div class="col-sm-6">
											...
											<?php
											$test = $row['flight_id'];
											$sqls = "SELECT price FROM flight_list WHERE id='$test'";
											$results = $conn->query($sqls);
											if ($results->num_rows > 0) {
												// output data of each row
												while ($rows = $results->fetch_assoc()) {
													$totalPayment = $rows["price"] * $row['Guest'];
													$totalPayment += $rows["price"];
												}
											}
											?>
											<p><small>Payment :<b><?php echo $totalPayment ?></small></b></p>
